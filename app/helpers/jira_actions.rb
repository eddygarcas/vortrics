module JiraActions
  ...
  def field_list project, options = {}
    return if current_user.setting.blank?
    Rails.cache.fetch("field_list_#{project}", expires_in: 7.day) {
      rest_query(jira_instance(current_user.setting), '/field', {}, options).map {|c| [c['id'], "#{c['name'].humanize} - #{c['id']}"]}
  ...
  def project_details key, options = {}
    ...
    Rails.cache.fetch("project_information_#{key.to_s}", expires_in: 7.day) {
      rest_query(jira_instance(current_user.setting), "/project/#{key}", {}, options)
    }
  end

  def current_project key, options = {}
    return if current_user.setting.blank?
    Rails.cache.fetch("get_current_project_#{key.to_s}", expires_in: 1.hour) {
  ...
  def import_sprint sprintId, options = {}
    return if current_user.setting.blank?
    Rails.cache.fetch("sprint_#{sprintId}", expires_in: 1.hour) {
      agile_query jira_instance(current_user.setting), "/sprint/#{sprintId}/issue", {}, options
    }[:issues.to_s]
  end

  def import_kanban boardId, options = {}
    return if current_user.setting.blank?
    Rails.cache.fetch("kanban_board#{boardId}", expires_in: 1.hour) {
      agile_query jira_instance(current_user.setting), "/board/#{boardId}/issue", {}, options
    }[:issues.to_s]
  end
  ...
  def bug_for_board boardid, startdate, enddate = nil, options = {}, status = nil
    return if current_user.setting.blank?
    Rails.cache.fetch("bug_for_board_#{boardid}_#{startdate.to_s}", expires_in: 1.day) {
      ...
      query_params = {:jql => parse_jql_params(param_hash)}
  ...
  def sprint_report boardid, sprintid, options = {}
    ...
    sprint_report = Rails.cache.fetch("sprint_report_#{sprintid.to_s}", expires_in: 1.day) {
      ...
      green_hopper_query jira_instance(current_user.setting), '/rapid/charts/sprintreport', param_hash, options
    }
  ...
  def boards_by_sprint board, startAt = 0, options = {}
    return if current_user.setting.blank?
    Rails.cache.fetch("boards_sprints_#{board.to_s}", expires_in: 1.day) {
      agile_query jira_instance(current_user.setting), "/board/#{board}/sprint", {startAt: startAt, toLast: 20}, options
    }['values']
  end
