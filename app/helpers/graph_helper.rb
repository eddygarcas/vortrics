module GraphHelper
  ...
  def self.number_stories_by_date issues, date = Time.now, no_stories = 0
    yday_issues = issues.select {|issue|
      issue.resolutiondate.yday.equal? date.yday unless issue.resolutiondate.blank? || issue.subtask? || issue.bug?
    }
  ...
  #This method won't retrive issues data because, takes for granted that all this data has been cached previously
  def self.number_stories_in_progress date = Time.now, issues
    in_progress_issues = issues.select {|issue|
      issue.updated.yday.equal? date.yday if issue.resolutiondate.blank? && issue.task?
    }
  ...
  def self.number_of items, date = Time.now, method = :created
    items.select {|item|
      item.send(method).to_datetime.yday.equal? date.yday unless item.send(method).blank?
    }.count
  end
