  ...
  class Client
    ...
    def initialize(token = nil, secret = nil)
      @instance = Trello::Client.new(
          ...
          :oauth_token => token.presence || current_user&.services.find_by_provider(:trello).access_token,
          :oauth_token_secret => secret.presence || current_user&.services.find_by_provider(:trello).access_token_secret
      )
    end
