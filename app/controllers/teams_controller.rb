class TeamsController < ApplicationController
  ...
  def graph_ratio_bugs_closed
    data = Rails.cache.fetch("graph_ratio_bugs_closed_#{@team.id}", expires_in: 30.minutes) {
      data = Array.new { Array.new }
      bugs = bugs_selectable_for_graph.map { |elem| Connect::Issue.new(elem) }.sort_by!(&:created_at)
      sum_open = 0
      sum_closed = 0
  ...
  def graph_lead_time_bugs
    data = Rails.cache.fetch("graph_lead_time_bugs_team_#{@team.id}", expires_in: 30.minutes) {
      data = Array.new { Array.new }
      bugs = bugs_selectable_for_graph.map { |elem| Connect::Issue.new(elem).to_issue }.sort_by!(&:created)
      data[0] = bugs.map.with_index { |issue, index| {x: index, y: issue.key} }
  ...
  def support
    @support_bugs = bugs_selectable_for_graph.map { |elem| Connect::Issue.new(elem).to_issue }.sort_by!(&:lead_time)
