class TeamsController < ApplicationController
  ...
  def graph_time_first_response
    render json: Rails.cache.fetch("graph_time_first_response_#{@team.id}", expires_in: 1.day) {
      data = Array.new { Array.new }
      bugs = bugs_first_comments boardid: @team.board_id
  ...
  def full_project_details
    render json: project_details(key: params['proj_id'])
  end

  def boards_by_team
    render json: boards_by_project(keyorid: params['proj_id'])['values'].map { |e| Board.new(e) }
  end
  ...
  def support
    ...
    @comments = bugs_first_comments boardid: @team.board_id, options: {fields: vt_jira_issue_fields, maxResults: 400}
  end
  ...
  def create
    ...
    @team.project_info_id = ProjectInfo.create_data(project_details(key: @team.project)).id
